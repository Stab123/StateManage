  ...
  <a-card :bordered="false">
    ...
    <div class="table-page-search-wrapper">
      <a-form layout="inline" @keyup.enter.native="searchQuery">
        <a-row :gutter="24">
          <a-col :span="6">
            <a-form-model-item label="元数据类型" >
              <a-select style="width: 150px"  v-model="selectedOption.metaDataType">
                <a-select-option  v-for="(metaDataTypeOpt,index) in metaDataTypeOptions" :value="metaDataTypeOpt" :key="index">
                  {{metaDataTypeOpt}}
                </a-select-option>
              </a-select>
            </a-form-model-item>
          ...
          <a-col :span="6">
            <a-form-model-item label="功能子域" >
              <a-select style="width: 150px"  v-model="selectedOption.subDomain">
                <a-select-option  v-for="(domainOpt,index) in subDomainOptions" :value="domainOpt" :key="index">
                  {{domainOpt}}
                </a-select-option>
              </a-select>
            </a-form-model-item>
...
export default {
  ...
  data() {
    return {
      ...
      selectedOption: {},
      // 元数据类型选项
      metaDataTypeOptions: ['运行数据', '监控数据', '全部'],
      // 功能域选项
  ...
  methods: {
    ...
    loadData(){
      this.selectedOption = {
        metaDataType: this.metaDataTypeOptions[0],
        domain : this.domainOptions[0],
        subDomain: this.subDomainOptions[0]
      }
      this.dataSource = [
        {
          key: '1',
